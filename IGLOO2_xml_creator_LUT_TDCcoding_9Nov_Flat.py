...
HB_tp1 = 12 # set values where LUT is zeroed out to -1, to completely eliminate prompt range / prompt veto
        ...
        for board in qie_board:
                for channel in qie_channel:
                        ...
                        if (irm == 5):
                            ...
                            t_p1 = HB_tp1
                            t_p2 = HB_tp1 + 2 # HB_tp2[int(eta)-1][int(depth)-1]
                            if t_p2 < 1:
                                t_p2 = 1 # last value of the prompt range (remains 1ns wide, even when zero-ed out)
...
def buildTree(site="904"):
  ...
  tree.write("HBTDCLUT_prompt_delayed_9Nov2022_Flat12_%s.xml" %site, xml_declaration=True, encoding='utf-8', method="xml")
